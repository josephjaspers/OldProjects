...
template<typename T, typename Math_lib, class lv, class rv, class operation>
struct buffer : accessor_ace<T, Math_lib> {
	...
	__attribute__((always_inline)) T operator [](int index) const {
		return operation::calc(left[index], right[index]);
	}


	template<typename eval_to>
	__attribute__((always_inline)) T operator ()(eval_to to, int index) const {
		return to[index] = left[index] + right[index];
	}
