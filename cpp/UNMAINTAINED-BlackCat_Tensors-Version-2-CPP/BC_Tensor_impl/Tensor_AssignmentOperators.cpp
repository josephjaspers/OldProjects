...
Tensor<number_type, TensorOperations>& Tensor<number_type, TensorOperations>::operator=(Tensor<number_type, TensorOperations> && cpy) {
	...
	if (isInitialized()) {
		...
		if (tensor_ownership) {
			...
			subTensor || cpy.subTensor ? TensorOperations::copy(tensor, ranks, order, ld, cpy.tensor, cpy.ld) : TensorOperations::copy(tensor,
													cpy.tensor, sz);
		}
	} else {
		sz = cpy.sz;
...
//
//	throw std::invalid_argument("initialzier_list - post init is no longer supported");
//	/*
//	 if (isInitialized()){
//	 if (size() != vector.size()) {
