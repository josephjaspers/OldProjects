...
inline __attribute__((always_inline)) void add(T* eval_to, T* a, T* b) {
	...
	eval_to[SIZE - 2] = a[SIZE - 2] + b[SIZE - 3];
	eval_to[SIZE - 3] = a[SIZE - 2] + b[SIZE - 4];

	add<SIZE - 4, T>(eval_to, a, b);
}
template<>
...
//	printf("It took me %d clicks (%f seconds).\n", t, ((float) t));
	...
	printf("\n Calculating... Eigen Manual for loop zeroing (iteration speed)) \n");
	...
	t = omp_get_wtime();
	printf("\n Calculating... Eigen Manual for loop adding 1  (iteration speed)) \n");
	for (int j = 0; j < reps; ++j)
		for (int i = 0; i < SIZE; ++i) {
			a1[i] = a1[i] + 1;
		}
