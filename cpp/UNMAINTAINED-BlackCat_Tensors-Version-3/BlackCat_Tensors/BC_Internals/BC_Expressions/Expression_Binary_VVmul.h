...
struct binary_expression_VVmul_outer : expression<T, binary_expression_VVmul_outer<T, COL_VECTOR, ROW_VECTOR>>{
	...
	typename COL_VECTOR::functor_type lv = vecL.data();
	typename ROW_VECTOR::functor_type rv = vecR.data();
	...
	T operator [] (int index) const {
		return lv[(int)std::ceil(index / vecL.rows())] * rv[(int)index % vecR.cols()];
	}
