...
namespace GPU_MATHEMATICS {
	...
	//unsigned helper functions
	void unified_initialize(unsigned*& d, unsigned sz);
	void destruction(unsigned* d);
	void port_copy(float* store, const float* v, unsigned sz);
	void port_copy_device_to_host(float* store, const float* v, unsigned sz);
	void port_copy_device_to_host(unsigned* store, const unsigned* v, unsigned sz);
	...
	__global__ void dot(float* store, unsigned store_LD, const float* m1, unsigned m1_row, unsigned m1_col, unsigned m1_inc, const float* m2, unsigned m2_row, unsigned m2_col, unsigned m2_inc);
	...
	void print(const float* ary, const unsigned* dims, const unsigned* lead_dims, unsigned index);

	//Advanced operations
	__global__ void axpy(float* s, const unsigned* s_ranks, unsigned order, const unsigned *s_LD, const float* m1, const unsigned* m1_LD, float scalar);
	__global__ void correlation(float* s, unsigned order, const unsigned* ranks, const float* filter, const unsigned* f_ld, const float* signal, const unsigned* s_ld);
	__global__ void cross_correlation(float* s, unsigned cor_mv, const unsigned* store_ld, const float* filter, const unsigned * f_ld, const unsigned* f_ranks, unsigned f_order, const float* signal, const unsigned * s_ld, const unsigned* s_ranks, unsigned s_order);
	...
	__global__ void cc_filter_error(unsigned move_dimensions, float* store, const unsigned* store_ld, const unsigned* store_ranks, unsigned store_order, const float* error, const unsigned* error_ld, const unsigned* error_ranks, unsigned error_order, const float* signal, const unsigned* signal_ld, const unsigned* signal_ranks, unsigned signal_order);
	__global__ void cc_signal_error(unsigned move_dimensions, float* store, const unsigned* store_ld, const unsigned* store_ranks, unsigned store_order, const float* error, const unsigned* error_ld, const unsigned* error_ranks, unsigned error_order, const float* signal, const unsigned* signal_ld, const unsigned* signal_ranks, unsigned signal_order);
