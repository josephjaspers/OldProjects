...
public:
	...
	template<typename number_type> static void dot(number_type* store, unsigned store_LD, const number_type* m1, unsigned m1_row, unsigned m1_col, unsigned m1_inc, const number_type* m2, unsigned m2_row, unsigned m2_col, unsigned m2_inc) {
		CPU_MATHEMATICS<number_type>::dot(store, store_LD, m1, m1_row, m1_col, m1_inc, m2, m2_row, m2_col, m2_inc);
	}

	template<typename number_type> static void zeros(number_type* x, unsigned sz) {
		CPU_MATHEMATICS<number_type>::zeros(x, sz);
	}
	...
	template<typename number_type> static void min_index(number_type* min_val, unsigned* min_indexes, const number_type* data, const unsigned* ranks, const unsigned* ld, unsigned order) {
		fill<number_type>(min_val, 0, 1);
		fill<unsigned>(min_indexes, 0, order);

		CPU_MATHEMATICS<number_type>::min_index(min_val, &min_indexes[order - 1], data, ranks, ld, order);
	}
	template<typename number_type> static void max_index(number_type* min_val, unsigned* min_indexes, const number_type* data, const unsigned* ranks, const unsigned* ld, unsigned order) {
		fill<number_type>(min_val, 0, 1);
		fill<unsigned>(min_indexes, 0, order);
		CPU_MATHEMATICS<number_type>::max_index(min_val, &min_indexes[order - 1], data, ranks, ld, order);
	}
	...
	template<typename number_type> static void cross_correlation_noAdjust(number_type* s, unsigned cor_mv, unsigned order, const unsigned* store_ld, const number_type* filter, const unsigned * f_ld, const unsigned* f_ranks, const number_type* signal, const unsigned * s_ld, const unsigned* s_ranks) {
		CPU_MATHEMATICS<number_type>::cross_correlation_noAdjust(s, cor_mv, order, store_ld, filter, f_ld, f_ranks, signal, s_ld, s_ranks);
	}

	template<typename number_type> static void cross_correlation(number_type* s, unsigned cor_mv, const unsigned* store_ld, const number_type* filter, const unsigned * f_ld, const unsigned* f_ranks, unsigned f_order, const number_type* signal, const unsigned * s_ld, const unsigned* s_ranks, unsigned s_order) {
		CPU_MATHEMATICS<number_type>::cross_correlation(s, cor_mv, store_ld, filter, f_ld, f_ranks, f_order, signal, s_ld, s_ranks, s_order);
	}
	template<typename number_type> static void cross_correlation_filter_error(unsigned cor_mv, number_type* s, const unsigned* store_ld, const unsigned* store_ranks, unsigned store_order, const number_type* filter, const unsigned * f_ld, const unsigned* f_ranks, unsigned f_order, const number_type* signal, const unsigned * s_ld, const unsigned* s_ranks, unsigned s_order) {
		CPU_MATHEMATICS<number_type>::cc_filter_error(cor_mv, s, store_ld, store_ranks, store_order, filter, f_ld, f_ranks, f_order, signal, s_ld,
				s_ranks, s_order);
	}
	template<typename number_type> static void cross_correlation_signal_error(unsigned cor_mv, number_type* s, const unsigned* store_ld, const unsigned* store_ranks, unsigned store_order, const number_type* filter, const unsigned * f_ld, const unsigned* f_ranks, unsigned f_order, const number_type* signal, const unsigned * s_ld, const unsigned* s_ranks, unsigned s_order) {
		CPU_MATHEMATICS<number_type>::cc_signal_error(cor_mv, s, store_ld, store_ranks, store_order, filter, f_ld, f_ranks, f_order, signal, s_ld,
				s_ranks, s_order);
	}
	...
	template<typename number_type> static void destruction(number_type* d) {
		CPU_MATHEMATICS<number_type>::destruction(d);
	}
	...
	template<typename number_type> static void port_copy(number_type* s, const number_type* m, unsigned sz) {
		CPU_MATHEMATICS<number_type>::copy(s, m, sz);
	}
	template<typename number_type> static void port_copy_device_to_host(number_type* s, const number_type* m, unsigned sz) {
		CPU_MATHEMATICS<number_type>::copy(s, m, sz);
	}
	...
	template<typename number_type> static void power(number_type* s, const number_type* m1, const number_type* m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::power(s, m1, m2, sz);
	}
	template<typename number_type> static void divide(number_type* s, const number_type* m1, const number_type* m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::divide(s, m1, m2, sz);
	}
	template<typename number_type> static void multiply(number_type* s, const number_type* m1, const number_type* m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::multiply(s, m1, m2, sz);
	}
	template<typename number_type> static void add(number_type* s, const number_type* m1, const number_type* m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::add(s, m1, m2, sz);
	}
	template<typename number_type> static void subtract(number_type* s, const number_type* m1, const number_type* m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::subtract(s, m1, m2, sz);
	}
	template<typename number_type> static void power(number_type* s, const number_type* m1, number_type m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::power(s, m1, m2, sz);
	}
	template<typename number_type> static void divide(number_type* s, const number_type* m1, number_type m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::divide(s, m1, m2, sz);
	}
	template<typename number_type> static void multiply(number_type* s, const number_type* m1, number_type m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::multiply(s, m1, m2, sz);
	}
	template<typename number_type> static void add(number_type* s, const number_type* m1, number_type m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::add(s, m1, m2, sz);
	}
	template<typename number_type> static void subtract(number_type* s, const number_type* m1, number_type m2, unsigned sz) {
		CPU_MATHEMATICS<number_type>::subtract(s, m1, m2, sz);
	}
