...
//initialize leading dimensions
	static void init_leading_dimensions(unsigned* leading_dim, unsigned* dim, unsigned order) {
...
//Shape equality for unordered map
	...
	struct shape_hasher {
		unsigned operator()(const std::vector<unsigned>& shape) const {
			unsigned hash = 0;
			unsigned tmp;
			for (unsigned i = 0; i < shape.size(); ++i) {
				hash += tmp << shape[i];
			}
			return hash << hash; //shift it over
		}
	};

	struct indexShape_pairEqual {
		bool operator()(const std::pair<std::vector<unsigned>, std::vector<unsigned>>& s1, const std::pair<std::vector<unsigned>,
				std::vector<unsigned>>& s2) const {
			return s1.first == s2.first && s1.second == s2.second;
		}
	};
	struct indexShape_pairHasher {
		unsigned operator()(const std::pair<std::vector<unsigned>, std::vector<unsigned>>& s1) const {
			unsigned hash = 0;

			for (unsigned i = 0; i < s1.first.size(); ++i) {
				hash += s1.first[i] * i;
			}
			for (unsigned i = 0; i < s1.second.size(); ++i) {
